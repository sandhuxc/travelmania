...
import { viewReqTourOrg } from '../../api/index';

const ReqTourOrg = ({ token }) => {
  console.log(token);
  let navigate = useNavigate();
  const [req, setReq] = useState();
  const getReq = async () => {
    let response = await viewReqTourOrg(token);
    if (response === 404) {
      navigate('/');
    }
            ...
            <small>
              ...
              <div>
                <Link
                  type='submit'
                  id='search'
                  to={'/admin/requests/tourorg/' + obj.req_guid}
                  name='search'
...
export default connect(mapStatetoProps, {})(ReqTourOrg);
